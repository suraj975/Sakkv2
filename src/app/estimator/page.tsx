...
function buildDummyEstimate(emirate: string, code: string, number: string) {
  ...
  const codeBoost = Math.max(0, 6 - Math.min(code.trim().length || 1, 5)) * 18000;
  const estimate = baseByDigits + emirateBoost + codeBoost + randomInt(15000, 220000);
            ...
            <Card padding="xl" className="space-y-8" style={{ boxShadow: "0 8px 28px rgba(25,28,29,0.08)" }}>
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <h2 className="text-5xl lg:text-6xl font-black tracking-tight" style={{ color: "var(--primary)" }}>
                    {aed(estimateResult.estimate)}
                  </h2>
                  <p className="text-sm font-bold" style={{ color: "var(--on-surface)" }}>
                    High estimate: <span style={{ color: "var(--on-surface-variant)" }}>{aed(estimateResult.highEstimate)}</span>
                  </p>
                </div>
                <div className="flex flex-wrap gap-2 pt-2">
                  <Badge variant="secondary">{emirate}</Badge>
                  <Badge variant="secondary">Code {code}</Badge>
                  <Badge variant="primary" className="!bg-[var(--teal-darker)] !text-white">
