...
import { ArrowLeft, Lock, Building2, CreditCard, Smartphone, ShieldCheck, Check } from "lucide-react";
...
const PAYMENT_METHODS = [
  { id: "bank", label: "Bank Transfer", sub: "Instant via UAE Central Bank", Icon: Building2 },
  { id: "card", label: "Credit Card", sub: "Visa, Mastercard, Amex", Icon: CreditCard },
  { id: "apple", label: "Apple Pay", sub: "Secure one-tap payment", Icon: Smartphone },
];
...
export default function CheckoutPage() {
  ...
  const handlePay = () => {
    ...
    setTimeout(() => router.push(`/plates/${plate.id}/checkout/complete`), 2000);
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto" style={{ background: "var(--surface)" }}>
      ...
      >
        ...
        <span className="font-black text-base" style={{ color: "var(--primary)" }}>Secure Checkout</span>
      ...
      <div className="max-w-5xl mx-auto px-4 lg:px-8 py-8 pb-32 lg:pb-16 space-y-6 lg:space-y-0 lg:grid lg:grid-cols-2 lg:gap-10 lg:items-start">
        ...
        <div className="space-y-5">
          <div>
            <h1 className="text-2xl lg:text-3xl font-black" style={{ color: "var(--on-surface)" }}>
          ...
          <Card padding="lg" className="space-y-5">
            <div className="flex justify-center py-2">
              <PlateViz code={plate.code} num={plate.num} emirate={plate.emirate} type={plate.type} size="md" />
            </div>
            <div className="flex items-center justify-between">
              ...
              <Badge variant="primary">{plate.verified ? "Verified" : "Listed"}</Badge>
            </div>
          </Card>

          {/* Order Summary */}
          <Card padding="md" className="space-y-3">
            <h3 className="text-[10px] font-black uppercase tracking-widest" style={{ color: "var(--outline)" }}>ORDER SUMMARY</h3>
            {[
              ...
              <div key={row.label} className="flex justify-between items-center">
                <span className="text-sm font-bold" style={{ color: "var(--on-surface-variant)" }}>{row.label}</span>
                <span className="text-sm font-black" style={{ color: "var(--on-surface)" }}>{row.value}</span>
              </div>
            ))}
            <div
            ...
            >
              <span className="text-base font-black" style={{ color: "var(--on-surface)" }}>Total</span>
              <span className="text-xl font-black" style={{ color: "var(--primary)" }}>{aed(total)}</span>
            </div>
          </Card>
          ...
          >
            ...
            <div>
              <p className="text-sm font-black" style={{ color: "var(--primary)" }}>100% Escrow Protected</p>
              <p className="text-[10px] font-medium" style={{ color: "var(--on-surface-variant)" }}>
        ...
        {/* Right: Payment */}
        <div className="space-y-5">
          <h3 className="text-lg font-black" style={{ color: "var(--on-surface)" }}>Payment Method</h3>
          ...
          <div className="space-y-3">
            {PAYMENT_METHODS.map(({ id, label, sub, Icon }) => (
              <button
                ...
                style={{
                  background: payMethod === id ? "var(--teal-light)" : "var(--surface-container-lowest)",
                  borderColor: payMethod === id ? "var(--primary)" : "rgba(187,202,199,0.2)",
                  fontFamily: "inherit",
                }}
              >
                <div
                  className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0"
                  style={{ background: payMethod === id ? "var(--primary)" : "var(--surface-container-low)" }}
                >
                  <Icon size={18} style={{ color: payMethod === id ? "white" : "var(--on-surface-variant)" }} />
                </div>
                <div className="flex-1">
                  <p className="text-sm font-black" style={{ color: "var(--on-surface)" }}>{label}</p>
                  <p className="text-[10px] font-bold" style={{ color: "var(--outline)" }}>{sub}</p>
                </div>
                {payMethod === id && (
                  <div
          ...
          <p className="text-center text-[10px] font-medium" style={{ color: "var(--outline)" }}>
            By proceeding you agree to Sakk's Terms of Service and Escrow Agreement.
          </p>
        </div>
      </div>
    </div>
